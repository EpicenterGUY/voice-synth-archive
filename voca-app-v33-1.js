/* VocaDive v39.27 · Producer Detail 2.0 + presets */
(function(){
"use strict";
...
function addProducerDetail2Style(){
  if(document.getElementById("v397ProducerDetailStyle"))return;
  var st=document.createElement("style");st.id="v397ProducerDetailStyle";st.textContent=".v397-prod-detail-top{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px}.v397-prod-back{min-height:34px;padding:0 10px;border:1px solid #285159;border-radius:10px;background:#0a282e;color:#abd0cb;font-size:8px;font-weight:900}.v397-prod-detail-top span{color:#658b87;font-size:7px}.v397-prod-tabs{position:sticky;top:0;z-index:8;display:flex;gap:5px;overflow-x:auto;margin:8px 0 10px;padding:6px;border:1px solid #1f454b;border-radius:13px;background:rgba(6,25,30,.95);backdrop-filter:blur(14px)}.v397-prod-tabs button{flex:0 0 auto;min-height:31px;padding:0 9px;border:1px solid #285159;border-radius:999px;background:#092329;color:#8fb7b3;font-size:7px;font-weight:900}.v397-prod-tabs button:first-child{background:#17444a;color:#effffc;border-color:#4e8585}.v397-prod-detail .v331-producer-stats{grid-template-columns:repeat(4,minmax(0,1fr))}.v397-prod-detail .v331-section{scroll-margin-top:60px}.v397-prod-detail .v331-producer-hero{padding:17px}body[data-vsa-theme='light'] .v397-prod-tabs{background:rgba(247,251,250,.96);border-color:#cfe2de}@media(max-width:699px){.v397-prod-detail .v331-producer-stats{grid-template-columns:1fr 1fr}.v397-prod-detail .v331-producer-hero{padding:14px}}";document.head.appendChild(st)
}
...
function section(key,title,sub,rows){
  return '<section class="v331-section" id="v397ProdSection-'+key+'"><div class="v331-section-head"><h3>'+title+'</h3><small>'+sub+'</small></div><div class="v331-songgrid">'+(rows.length?rows.slice(0,8).map(songCard).join(""):'<div class="v33-card-body">아직 이 프로듀서의 곡을 찾지 못했어요.</div>')+'</div></section>';
}
...
async function openProducer(tag){
  ...
  var peak=all.length?Math.max.apply(null,all.map(function(x){return Number(x.viewCounter)||0})):0;
  root.classList.add("v397-prod-detail");
  root.innerHTML='<div class="v397-prod-detail-top"><button type="button" class="v397-prod-back" data-v397-prod-back>‹ P 목록</button><span>PRODUCER DETAIL 2.0</span></div>'+
    '<div class="v331-producer-hero"><small>PRODUCER 2.0 · P명 확인</small><h2>'+esc(tag)+'</h2><p>니코니코의 P명·설명·태그와 VocaDB 역할 정보를 함께 대조해 대표곡과 활동 흐름을 정리했습니다.</p><div class="v331-producer-evidence">'+aliases.map(function(x){return'<span>'+esc(x)+'</span>'}).join("")+'<span class="source">Niconico</span><span class="source">VocaDB</span></div><div class="v331-producer-actions"><button class="primary" data-v331-search="'+esc(searchName)+'">전체 곡 검색</button><button data-v331-follow="'+esc(tag)+'">'+(followed(tag)?"팔로우 해제":"♡ 팔로우")+'</button></div></div>'+
    '<div class="v331-producer-stats"><div class="v331-producer-stat"><small>확인 곡</small><b>'+all.length+'</b></div><div class="v331-producer-stat"><small>누적 조회</small><b>'+fmt(views)+'</b></div><div class="v331-producer-stat"><small>최고 조회</small><b>'+fmt(peak)+'</b></div><div class="v331-producer-stat"><small>최신 활동</small><b>'+(recent[0]&&recent[0].startTime?new Date(recent[0].startTime).getFullYear():"-")+'</b></div></div>'+
    '<nav class="v397-prod-tabs"><button type="button" data-v397-prod-tab="popular">대표곡</button><button type="button" data-v397-prod-tab="recent">최근곡</button><button type="button" data-v397-prod-tab="deep">숨은 곡</button></nav>'+
    section("popular","대표곡","조회수 중심",popular)+section("recent","최근곡","최근 등록순",recent)+section("deep","숨은 곡","5만 조회 이하 반응률 중심",deep);
}
...
function bind(){
  document.addEventListener("click",function(e){
    ...
    var backProd=e.target.closest&&e.target.closest("[data-v397-prod-back]");if(backProd){api()&&api().openView("explore33");return}var prodTab=e.target.closest&&e.target.closest("[data-v397-prod-tab]");if(prodTab){var sec=document.getElementById("v397ProdSection-"+prodTab.dataset.v397ProdTab);if(sec)sec.scrollIntoView({behavior:"smooth",block:"start"});return}var sr=e.target.closest&&e.target.closest("[data-v331-search]");if(sr){api()&&api().searchProducer(sr.dataset.v331Search);return}
  });
}
...
var __booted=false;
function boot(){if(__booted)return;__booted=true;addStyle();addProducerDetail2Style();ensureProducerView();buildPresetSheet();renderPresets();installHomeTabs();observeHome();bind();window.VSAV33ProducerDetail=openProducer;setTimeout(function(){renderPresets();installHomeTabs()},500)}
