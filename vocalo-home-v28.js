...
function keep(id){var s=find(id);if(!s)return;var db=load(OK,{version:1,library:{},recentViews:[],recentSearches:[],cases:[],compare:[],snapshots:{}});db.library=db.library||{};db.library[id]={status:"interest",song:norm(s),savedAt:Date.now(),updatedAt:Date.now()};save(OK,db);try{if(window.VSAOrganizer22&&window.VSAOrganizer22.renderLibrary)window.VSAOrganizer22.renderLibrary()}catch(e){}try{toast("관심곡으로 보관했습니다.")}catch(e){}}
...
async function loadFeed(force){if(busy)return;busy=true;var root=document.getElementById("v28Home");if(force&&root)root.innerHTML='<div class="v28-empty">추천 피드를 새로 계산하는 중…</div>';try{var c=load(CK,null);if(!force&&c&&c.date===day()){render(c);return}if(typeof relayBase==="function"&&!relayBase()){var p=pool(),seed=hash(day()),pm=pref(),f={date:day(),at:Date.now(),daily:shuffle(p,seed).slice(0,12),taste:p.slice().sort(function(a,b){return tasteScore(b,pm)-tasteScore(a,pm)}).slice(0,12),newer:p.filter(function(s){return s.startTime}).sort(function(a,b){return new Date(b.startTime)-new Date(a.startTime)}).slice(0,12),hidden:p.filter(function(s){return s.viewCounter>0&&s.viewCounter<=50000}).sort(function(a,b){return hiddenScore(b)-hiddenScore(a)}).slice(0,12),signals:pm.slice(0,8).map(function(x){return x[0]})};render(f);return}render(await gather())}catch(e){if(root)root.innerHTML='<div class="v28-empty">추천 피드를 만들지 못했습니다.</div>'}finally{busy=false}}
...
function boot(){addStyle();build();clicks();polish();loadFeed(false);setTimeout(polish,300)}
