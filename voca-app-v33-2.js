/* VocaDive v39.27 · Library 3.0 + feed more */
(function(){
"use strict";
...
function addLibrary3Style(){
  if(document.getElementById("v397LibraryStyle"))return;
  var st=document.createElement("style");st.id="v397LibraryStyle";st.textContent=".v397-lib-hero{display:flex;align-items:flex-end;justify-content:space-between;gap:14px;padding:16px;margin-bottom:9px;border:1px solid rgba(113,205,197,.13);border-radius:20px;background:radial-gradient(circle at 90% 0,rgba(107,119,238,.15),transparent 32%),linear-gradient(145deg,#0c2d34,#071d23)}.v397-lib-hero small{display:block;color:#6fd9d0;font-size:7px;font-weight:950;letter-spacing:.13em}.v397-lib-hero h2{margin:5px 0 4px;font-size:26px}.v397-lib-hero p{max-width:650px;margin:0;color:#779a96;font-size:8px}.v397-lib-hero-actions{display:flex;gap:6px}.v397-lib-hero-actions button{min-height:37px;padding:0 10px;border:1px solid #2d5960;border-radius:11px;background:#0b2b31;color:#c7e4e0;font-size:8px;font-weight:900}.v397-lib-hero-actions .primary{border:0;background:linear-gradient(135deg,#69d9cf,#768ff1);color:#061719}.v397-lib-stats{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr));gap:7px!important;margin-bottom:10px!important}.v397-lib-stat{padding:9px 10px;border:1px solid #20474e;border-radius:13px;background:#081e23}.v397-lib-stat small{display:block;color:#668d89;font-size:6.5px}.v397-lib-stat b{display:block;margin-top:3px;font-size:15px}.v397-lib-stat span{display:block;margin-top:2px;color:#62827e;font-size:6.5px}.v397-lib-toolbar{padding:9px;border:1px solid #1e4147;border-radius:15px;background:#071c21;margin-bottom:8px}.v397-lib-toolbar .v332-lib-controls{margin:0}.v397-lib-toolbar .v332-lib-chips{padding:7px 0 0}.v397-lib-title-row{display:flex;justify-content:space-between;align-items:center;margin:8px 1px 7px}.v397-lib-title-row h3{margin:0;font-size:14px}.v397-lib-title-row small{color:#678c88;font-size:7px}body[data-vsa-theme='light'] .v397-lib-hero,body[data-vsa-theme='light'] .v397-lib-toolbar{background:#fff;border-color:#cfe2de;color:#173431}body[data-vsa-theme='light'] .v397-lib-stat{background:#f4f8f7;border-color:#cfe2de;color:#173431}@media(max-width:699px){.v397-lib-hero{align-items:flex-start;flex-direction:column;padding:14px}.v397-lib-hero-actions{width:100%}.v397-lib-hero-actions button{flex:1}.v397-lib-stats{grid-template-columns:1fr 1fr!important}}";document.head.appendChild(st)
}
...
function ensureView(){
  ...
  el.innerHTML='<div class="v397-lib-hero"><div><small>MY LIBRARY 3.0</small><h2>내 보관함</h2><p>저장한 곡을 상태별로 정리하고, 지금 보고 있는 조건 안에서 바로 랜덤 재생하거나 최근 기록으로 돌아갈 수 있습니다.</p></div><div class="v397-lib-hero-actions"><button type="button" data-v397-lib-recent>최근 기록</button><button type="button" class="primary" data-v397-lib-random>랜덤 1곡 ▶</button></div></div>'+
    '<div class="v332-lib-stats v397-lib-stats" id="v332LibStats"></div>'+
    '<div class="v397-lib-toolbar"><div class="v332-lib-controls"><input id="v332LibQuery" placeholder="곡명 · 태그로 보관함 검색"><select id="v332LibSort"><option value="updated">최근 변경</option><option value="views">조회수</option><option value="title">제목</option></select><button id="v332SelectMode">여러 곡 선택</button></div><div class="v332-lib-chips" id="v332LibChips"></div></div>'+
    '<div class="v397-lib-title-row"><h3>곡</h3><small id="v397LibResultCount">0곡</small></div><div class="v332-lib-grid" id="v332LibGrid"></div>'+
...
function render(){
  ...
  stats.innerHTML='<div class="v397-lib-stat"><small>전체</small><b>'+c.all+'</b><span>정리된 곡</span></div><div class="v397-lib-stat"><small>최애</small><b>'+c.favorite+'</b><span>추천 강하게 반영</span></div><div class="v397-lib-stat"><small>관심</small><b>'+c.interest+'</b><span>다시 볼 곡</span></div><div class="v397-lib-stat"><small>나중에 조사</small><b>'+c.investigate+'</b><span>더 찾아볼 곡</span></div>';
  var a=records();grid.parentElement.classList.toggle("v332-selecting",selectMode);
  grid.innerHTML=a.length?a.map(card).join(""):'<div class="v332-empty">이 조건에 맞는 곡이 없어요.</div>';var rc=document.getElementById("v397LibResultCount");if(rc)rc.textContent=a.length+"곡"+(filter!=="all"?" · "+statusText(filter):"");
  ...
  var b=document.getElementById("v332SelectMode");if(b)b.textContent=selectMode?"선택 종료":"여러 곡 선택";
}
...
function randomLibraryPlay397(){var a=records().filter(function(x){return x.status!=="dislike"});if(!a.length){try{toast("현재 조건에서 재생할 보관곡이 없습니다.")}catch(_){}return}var x=a[Math.floor(Math.random()*a.length)];play(x.id)}
function openRecent397(){if(window.VSAOrganizer22&&window.VSAOrganizer22.openLibrary){window.VSAOrganizer22.openLibrary("recent");return}try{if(window.VSAV33&&VSAV33.openView)VSAV33.openView("library22")}catch(_){}}
function buildSheet(){
...
function bind(){
  document.addEventListener("click",function(e){
    var recent397=e.target.closest&&e.target.closest("[data-v397-lib-recent]");if(recent397){openRecent397();return}var random397=e.target.closest&&e.target.closest("[data-v397-lib-random]");if(random397){randomLibraryPlay397();return}var f=e.target.closest&&e.target.closest("[data-lib-filter]");if(f){filter=f.dataset.libFilter;selected.clear();render();return}
...
var __booted=false;
function boot(){if(__booted)return;__booted=true;addStyle();addLibrary3Style();ensureView();ensureFeedView();buildSheet();render();patchHome();bind();observe();window.VSA332Library={open:function(){if(window.VSAV33&&window.VSAV33.openView)window.VSAV33.openView("libraryHub33");setTimeout(render,0)},render:render};setTimeout(patchHome,400)}
