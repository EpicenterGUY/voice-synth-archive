...
function ensureRoot(){
  var old=document.getElementById("v28Home");if(!old)return null;
  var root=document.getElementById("v35Home");
  if(!root){root=document.createElement("main");root.id="v35Home";old.insertAdjacentElement("afterend",root)}
  return root;
}
...
function render(){
  ...
  root.innerHTML='<div class="v35-head"><div><small>VOCALO SUPPORT · '+esc(date)+'</small><h1>오늘의 보카로</h1><p>한 화면에서는 한 피드만. 필요한 추천만 골라서 봅니다.</p></div></div>'+
    ...
    (hero?'<section class="v35-feature"><a href="https://www.nicovideo.jp/watch/'+encodeURIComponent(hero.contentId)+'" target="_blank" rel="noopener">'+(hero.thumbnailUrl?'<img src="'+esc(hero.thumbnailUrl)+'" loading="eager" fetchpriority="high" decoding="async" alt="">':'')+'<div class="v35-feature-shade"></div><div class="v35-feature-copy"><small>'+label(active)+' PICK</small><b>'+esc(hero.title||hero.contentId)+'</b><span>조회 '+fmt(hero.viewCounter||0)+' · '+esc(reason(hero,active))+'</span></div><span class="v35-feature-play">▶</span></a></section>':'')+
...
function observe(){
  ...
  var old=document.getElementById("v28Home");if(old)new MutationObserver(function(){requestAnimationFrame(render)}).observe(old,{childList:true});
}
function boot(){addStyle();ensureRoot();render();bind();observe();window.VSAHome35={render:render,refresh:refresh}}
