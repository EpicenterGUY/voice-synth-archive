...
async function warmFromClues(env,body){
  if(!await dbReady(env))return {added:0};
...
async function handleDetectiveSearch(request,env){
  ...
  let rows=await lexicalSearch(env,body);
  let warmed=0;
  if(rows.length<12){
    const w=await warmFromClues(env,body);
    warmed=w.added;
    if(warmed)rows=await lexicalSearch(env,body);
  }
  const count=await dbCount(env);
  return json({
    ok:true,indexReady:true,indexVersion:INDEX_VERSION,indexedSongs:count,
    warmed,candidates:rows,
    mode:rows.length?"local-index":"index-empty"
  });
}
...
async function syncVocaDBPage(env){
  ...
  try{
    ...
    return {ok:false,reason:String(e?.message||e),start};
  }
  if(!items.length)start=0;
  let added=0;
  for(const item of items){
    try{await upsertEntity(env,vocadbToEntity(item));added++}catch{}
  }
  const next=items.length?start+items.length:0;
  await env.DB.prepare(`
    INSERT INTO sync_state(key,value,updated_at) VALUES(?,?,?)
    ON CONFLICT(key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at
  `).bind(key,String(next),new Date().toISOString()).run();
  return {ok:true,start,next,added};
}
...
export default {
  async fetch(request,env,ctx) {
    ...
    if(u.pathname==="/" || u.pathname==="/health"){
      return json({
        ok:true,service:"voice-synth-archive-worker",version:"16.0",
    ...
    if(u.pathname==="/detective/status"){
      const ready=await dbReady(env);
      return json({ok:true,indexReady:ready,indexVersion:INDEX_VERSION,indexedSongs:ready?await dbCount(env):0});
    }
    ...
    if(!out.searchParams.has("_context"))out.searchParams.set("_context","voice_synth_archive_v16");
    ...
    try{
      const r=await fetch(out.toString(),{
        headers:{"Accept":"application/json","User-Agent":"voice-synth-niconico-archive/16.0"}
      });
  ...
  async scheduled(event,env,ctx){
    ctx.waitUntil(syncVocaDBPage(env));
  }
};
